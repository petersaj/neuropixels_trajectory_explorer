...
%% GUI setup
function neuropixels_trajectory_explorer_mpm
...
yyaxis(gui_data.handles.axes_probe_areas(gui_data.selected_probe),'right');
set(gui_data.handles.probe_areas_plot(gui_data.selected_probe),'YData',probe_coords_depth,'CData',probe_areas); 
set(gui_data.handles.axes_probe_areas(gui_data.selected_probe),'YTick',probe_area_centers,'YTickLabels',probe_area_labels);
...
axes_probe_areas_xpos = 0.7 + (new_probe_idx-1)*0.1;
axes_probe_areas = axes(probe_atlas_gui,'Position',[axes_probe_areas_xpos,0.1,0.03,0.8]);
axes_probe_areas.ActivePositionProperty = 'position';
set(axes_probe_areas,'FontSize',11);
yyaxis(axes_probe_areas,'left');
probe_areas_plot = image(axes_probe_areas,0);
set(axes_probe_areas,'XTick','','YLim',[0,probe_length],'YColor','k','YDir','reverse');
if new_probe_idx == 1
    ylabel(axes_probe_areas,'Depth (mm)');
else
    set(axes_probe_areas,'YTick',[]);
end
yyaxis(axes_probe_areas,'right');
set(axes_probe_areas,'XTick','','YLim',[0,probe_length],'YColor','k','YDir','reverse');
title(axes_probe_areas,sprintf('Probe %d',new_probe_idx));
colormap(axes_probe_areas,gui_data.cmap);
caxis(axes_probe_areas,[1,size(gui_data.cmap,1)]);

% Set up click-to-select (probe line or area axes)
set(probe_line,'ButtonDownFcn',{@select_probe,probe_atlas_gui});
set(probe_areas_plot,'ButtonDownFcn',{@select_probe,probe_atlas_gui});
...
gui_data.handles.axes_probe_areas(new_probe_idx) = axes_probe_areas; % Axes with probe areas
gui_data.handles.probe_areas_plot(new_probe_idx) = probe_areas_plot; % Color-coded probe regions
...
delete(gui_data.handles.axes_probe_areas(gui_data.selected_probe));
delete(gui_data.handles.probe_areas_plot(gui_data.selected_probe));
...
gui_data.handles.axes_probe_areas(gui_data.selected_probe) = [];
gui_data.handles.probe_areas_plot(gui_data.selected_probe) = [];

% Update guidata
guidata(probe_atlas_gui,gui_data);

% Select first probe (if there is one)
if length(gui_data.handles.probe_line) > 0
...
if eventdata.Source.Value
    ...
    % Clear all user probes, add all MPM probes
    for curr_user_probe = 1:length(gui_data.handles.probe_line)
        probe_remove([],[],probe_atlas_gui);
        gui_data = guidata(probe_atlas_gui);
    end
    for curr_mpm_probe = 1:mpm_n_probes
        probe_add([],[],probe_atlas_gui);
        gui_data = guidata(probe_atlas_gui);
    end
...
for i = 1:length(gui_data.handles.axes_probe_areas)
    yyaxis(gui_data.handles.axes_probe_areas(i),'left');
end
set(gui_data.handles.axes_probe_areas(selected_probe_idx),'YColor',selected_color);
set(gui_data.handles.axes_probe_areas(~selected_probe_idx),'YColor',unselected_color);
